...
function diff($old, $new){
	...
	$op=array();
	foreach(explode("\n", $diff) as $ln){
		if(preg_match('#^(\d+)(?:,(\d+))?([cda])#', $ln, $matches)){
			list($orig, $start, $end, $act)=$matches;
			if($op) $ops[]=$op;
			$op=array('start'=>$start, 'end'=>$end, 'op'=>$act, 'lines'=>array());
		}elseif($ln!='---'){
			$op['lines'][]=$ln;
		}
	}
	...
	foreach($ops as $op){
		...
		switch($op['op']){
			case 'c':
			case 'd':
				$prev=array_slice($orig, 0, $start-1);
				$next=array_slice($orig, $end+1);
				break;
			case 'a':
				$prev=array_slice($orig, 0, $start);
				$next=array_slice($orig, $start);
				break;
		}
		$orig=array_merge($prev, $curr, $next);
	}
	return implode("\n", $orig);
}